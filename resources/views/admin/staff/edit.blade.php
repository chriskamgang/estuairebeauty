        ...
        <div class="bg-dark-400 rounded-xl border border-dark-100 p-6">
            <form action="{{ route('admin.staff.update', $staff) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="role" id="role" value="{{ old('role', $staff->role) }}"
                        class="w-full px-4 py-3 bg-dark-300 border border-dark-100 rounded-lg text-white focus:outline-none focus:border-gold">
                </div>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="bio" id="bio" rows="3"
                        class="w-full px-4 py-3 bg-dark-300 border border-dark-100 rounded-lg text-white focus:outline-none focus:border-gold">{{ old('bio', $staff->bio) }}</textarea>
                </div>
                ...
                <div class="mb-5">
                    ...
                    @if($staff->photo)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $staff->photo) }}" alt="{{ $staff->name }}" class="w-20 h-20 rounded-full object-cover">
                        </div>
                    @endif
                ...
                <div class="mb-6">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $staff->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded bg-dark-300 border-dark-100 text-gold focus:ring-gold">
